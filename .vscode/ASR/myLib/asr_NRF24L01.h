...
/**
 * @brief Construct a new NRF24L01::NRF24L01 object
 * @param  ce               CE引脚
 * @param  csn              csn引脚
 * @param  irq              irq引脚
 * @param  sck              sck引脚
...
/**
 * @brief 根据SPI协议，写一字节数据到nRF24L01，同时从nRF24L01读出一字节.
 * @param  dat              写入的数据
 * @return  读出的字节数据
 */
...
{
	uint8_t status;
	digitalWrite (_CSN_pin, 0);
	// CSN置低，开始传输数据
	status = SPI_RW (reg);		// 选择寄存器，同时返回状态字
	SPI_RW (value);				// 然后写数据到该寄存器
	digitalWrite (_CSN_pin, 1);		// CSN拉高，结束数据传输
...
uint8_t NRF24L01::SPI_Read(uint8_t reg)
{
	...
	digitalWrite (_CSN_pin, 0);		// CSN置低，开始传输数据
	SPI_RW (reg);				// 选择寄存器
	reg_val = SPI_RW (NOP);		// 然后从该寄存器读数据
	digitalWrite (_CSN_pin, 1);		// CSN拉高，结束数据传输
	return (reg_val);			// 返回寄存器数据
}

/**
 * @brief 从reg寄存器读出bytes个字节，通常用来读取接收通道数据或接收/发送地址.
 * @param  reg              寄存器 
 * @param  pBuf             返回的数据指针
...
uint8_t NRF24L01::SPI_Read_Buf (uint8_t reg, uint8_t * pBuf, uint8_t bytes)
{
	...
	for (i = 0; i < bytes; i++)
    {
		pBuf[i] = SPI_RW (NOP);	// 逐个字节从nRF24L01读出
    }
	digitalWrite (_CSN_pin, 1);		// CSN拉高，结束数据传输
	return (status);			// 返回状态寄存器
}
...
/**
 * @brief 把pBuf缓存中的数据写入到nRF24L01，通常用来写入发射通道数据或接收
 * @param  reg              寄存器
 * @param  pBuf             写入的数据指针
...
/**
 * @brief 设置无线接收通道
 * @param  channel          通道(0-125) 频率计算公式：2400+channel (Mhz)
 */
...
void NRF24L01::rx_setup ()
{
    ...
    //SPI_RW_Reg (WRITE_REG + CONFIG, 0x03);	//不启用CRC

	digitalWrite (_CE_pin, 1);// 拉高CE启动接收设备
    delay(300);
}
...
void NRF24L01::tx_setup()
{
    ...
    //SPI_RW_Reg (WRITE_REG + CONFIG, 0x02);//不启用CRC

	digitalWrite (_CE_pin, 1);//CE为高,10us后启动发送
    delay(300);
}

/**
 * @brief 无线发送32个字节数据(程序设置好了，字节数必须为32)
 * @param  BUF              数组指针
 */
void  NRF24L01::write(uint8_t * BUF)
{
    ...
    if(sta&MAX_RT){
         SPI_RW_Reg(FLUSH_TX, 0xff);  // 清除TX-FIFO
     }
}
...
/**
 * @brief 无线接收存入内部缓存中(最多接收32字节)，调用此函数以下函数可用
 * @return 1,接收到数据;0,没有接收到数据. 
 */
